...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        try {
            ...
            await global.db.set(`user.${user.replace(/@.*|:.*/g, "")}.isBanned`, true);

            ctx.sendMessage(user, {
                text: quote(`🎉 ${await global.tools.msg.translate("Anda telah dibanned oleh Owner!", userLanguage)}`)
            });
            return ctx.reply(quote(`✅ ${await global.tools.msg.translate("Berhasil dibanned!", userLanguage)}`));
        } catch (error) {
