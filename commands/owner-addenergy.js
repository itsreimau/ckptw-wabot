...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        const energyAmount = parseInt(ctx.args[1], 10);
        ...
        if (!ctx.args.length || !user || isNaN(energyAmount)) return ctx.reply({
        ...
        try {
            ...
            await global.db.add(`user.${user.split("@")[0]}.energy`, energyAmount);

            ctx.sendMessage(user, {
                text: quote(`🎉 Anda telah menerima ${energyAmount} energi dari Owner!`)
            });
            return ctx.reply(quote(`✅ Berhasil menambahkan ${energyAmount} energi kepada pengguna!`));
        } catch (error) {
