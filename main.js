...
bot.ev.on(Events.MessagesUpsert, async (m, ctx) => {
    ...
    if (prefixRegex.test(content)) {
        ...
        if (mean && mean !== cmdName) {
            return await ctx.reply(quote(`❓ ${await tools.msg.translate("Apakah maksud Anda", userLanguage)} ${monospace(prefix + mean)}?`));
        }
    }
...
async function sendMenfess(ctx, m, senderNumber, from) {
    const fakeText = {
        ...
        message: {
            extendedTextMessage: {
                text: await await tools.msg.translate(`${senderNumber} telah merespons pesan menfess Anda.`, userLanguage),
    ...
    await ctx.sendMessage(
        from + S_WHATSAPP_NET, {
            text: `${m.content}\n` +
                `${global.msg.readmore}\n` +
                await await tools.msg.translate("Jika ingin membalas, Anda harus mengirimkan perintah lagi.", userLanguage),
            contextInfo: {
        ...
        if (getWelcome) {
            ...
            for (const jid of participants) {
                ...
                const message = m.eventsType === "UserJoin" ?
                    quote(`⚠ ${await await tools.msg.translate(`Selamat datang @${jid.replace(/@.*|:.*/g, "")} di grup ${metadata.subject}!`, userLanguage)}`) :
                    quote(`⚠ ${await await tools.msg.translate(`@${jid.replace(/@.*|:.*/g, "")} keluar dari grup ${metadata.subject}.`, userLanguage)}`);
