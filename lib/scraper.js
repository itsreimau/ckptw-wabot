...
/**
 * Perform inference using the inference engine API.
 * @param {string} urlPath The URL path of the image to be processed.
 * @param {string} [method='enhance'] The method for inference ('enhance', 'recolor', or 'dehaze').
 * @returns {Promise<any>|null} The result of the inference operation.
 */
exports.inferenceengine = async (urlPath, method) {
    try {
        const validMethods = ['enhance', 'recolor', 'dehaze'];
        method = validMethods.includes(method) ? method : validMethods[0];

        const scheme = createAPIUrl('https://inferenceengine.vyro.ai', `/${method}`, {});
        const formData = new FormData();
        formData.append('model_version', 1, {
            contentType: 'binary'
        });
        formData.append('image', Buffer.from(urlPath), {
            filename: 'enhance_image_body.jpg',
            contentType: 'image/jpeg'
        });

        const response = await axios.post(scheme, formData, {
            headers: {
