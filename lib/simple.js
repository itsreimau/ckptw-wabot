...
exports.isAdmin = async (ctx) => {
    ...
    if (groupMetadata) {
        ...
        const participant = participants.find(participant => participant.id.split('@')[0] === senderId);
...
exports.isAdminOf = async (ctx) => {
    ...
    if (groupMetadata) {
        const participants = groupMetadata.participants;
        const botId = ctx._client.user.id.split(':')[0];
        const participant = participants.find(participant => participant.id.split('@')[0] === botId);
