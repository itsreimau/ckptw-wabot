...
exports.isAdmin = async (ctx) => {
    ...
    if (groupMetadata) {
        ...
        const senderId = ctx._sender.jid.split('@')[0];
        const participant = participants.find(participant => participant.jid.split('@')[0] === senderId);
...
exports.isAdminOf = async (ctx) => {
    ...
    if (groupMetadata) {
        const participants = groupMetadata.participants;
        const botId = ctx._client.user.jid.split('@')[0];
        const participant = participants.find(participant => participant.jid.split('@')[0] === botId);
