...
async function checkAdmin(ctx, id) {
    ...
    return group.participants.filter(v => v.admin === "admin" && v.id == id).length ? true : false
}
...
// formatSize: Function to format file size into human-readable format.
/**
...
exports.isAdminOf = async (ctx) => {
    const isAdminOfGroup = await checkAdmin(ctx, `${ctx._client.user.id.split(':')[0]}@s.whatsapp.net`);
    return isAdminOfGroup ? 1 : 0;
}

/**
 * Function to check if the context is in a group.
 * @param {object} ctx - The context object.
 * @returns {number} Returns 1 if the context is in a group, otherwise returns 0.
 */
exports.isGroup = (ctx) => {
    return ctx.isGroup() ? 1 : 0;
}
...
/**
 * Function to check if the context is in a private.
 * @param {object} ctx - The context object.
 * @returns {number} Returns 1 if the context is in a private, otherwise returns 0.
 */
exports.isPrivate = (ctx) => {
    return ctx.isGroup() ? 0 : 1;
}
